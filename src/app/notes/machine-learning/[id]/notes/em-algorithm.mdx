...
However, the above equation is not concave with respect to $\theta$. Hence, we cannot use gradient ascent to find the maximum likelihood estimate.
...
We can verify that setting $Q(z) = p(z | x; \theta)$ leads to a tight bound:
...
<Figure src="/images/notes/em-lower-bound.png" alt="The ELBO is tangent to the log-likelihood at theta-t; maximizing it reaches theta-(t+1), where the log-likelihood is higher still">
    For fixed $Q^{(t)}$, the $\text{ELBO}$ touches $\ell$ at $\theta^{(t)}$ (A); the $M$-step maximizes it to $\theta^{(t+1)}$ (B), and since $\ell \geq \text{ELBO}$, $\ell(\theta^{(t+1)})$ (C) is higher still — so a step never decreases $\ell$.
</Figure>
...
Finally, in the $E$ step, $Q(z)$ is chosen such that for the current value of $\theta$, the log-likelihood and the evidence lower bound are equal.

$$
\sum_{i=1}^{n} \text{ELBO}(x^{(i)}; Q_i^{(t)}, \theta^ {(t)}) = \ell(\theta^{(t)})
$$

Putting it all together, we have the following inequality:
...
<Figure src="/images/notes/em-staircase.png" alt="Successive tangent lower bounds climbing the log-likelihood toward a local maximum, with shrinking steps">
    Repeating the step, each fresh bound is tangent to $\ell$ at the current $\theta^{(k)}$ and is maximized to reach the next iterate; the iterates climb $\ell$ with shrinking steps, converging to a local maximum.
</Figure>
...
<Figure src="/images/notes/em-elbo-gap.png" alt="Log-likelihood split into the ELBO plus the KL gap; setting Q to the posterior closes the gap so the bound is tight">
    At a fixed $\theta$, $\log p(x; \theta)$ splits into the $\text{ELBO}$ and the gap $D_{KL}(Q \parallel p_{z | x})$; the $E$ step sets $Q = p(z \mid x)$, closing the gap so the bound is tight.
</Figure>
