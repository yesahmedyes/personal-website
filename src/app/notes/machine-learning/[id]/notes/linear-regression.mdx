...
<Figure src="/images/notes/linear-regression-residuals.png" alt="Linear regression fit with vertical residuals from each data point to the fitted line" className="py-20">
...
<Figure src="/images/notes/linear-regression-cost.png" alt="Gradient descent path over contour lines of the linear regression cost function">
    Gradient descent updates $\theta$ by moving downhill along the contours of $J(\theta)$ until it reaches the minimum.
</Figure>
...
<Figure src="/images/notes/linear-regression-projection.png" alt="Orthogonal projection of the target vector onto the column space of the design matrix">
    The closed-form solution chooses the prediction vector $\hat{y} = X\theta$ that is closest to $\vec{y}$ inside the column space of $X$.
</Figure>
