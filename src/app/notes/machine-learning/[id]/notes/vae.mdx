...
However, for more complex models like the Variational Autoencoder, $z$ is continuous. Therefore, it is intractable to compute $p(z | x; \theta)$ explicitly. Instead, we will try to find an approximation of $p(z | x; \theta)$.
...
<Figure src="/images/notes/vae-architecture.png" alt="An encoder maps x to a mean and variance, a latent z is sampled from them, and a decoder maps z back to a reconstruction">
    The encoder turns $x$ into the parameters of $Q$ — a mean $q(x; \phi)$ and a variance $v(x; \psi)$; we sample a latent $z \sim Q$ and the decoder $g(z; \theta)$ maps it back to a reconstruction $\hat{x}$.
</Figure>
...
<Figure src="/images/notes/vae-reparameterization.png" alt="A standard normal distribution shifted and scaled by mu and sigma into the latent distribution">
    Sampling $z \sim \mathcal{N}(\mu, \sigma^2)$ is equivalent to drawing $\xi \sim \mathcal{N}(0, 1)$ and mapping $z = \mu + \sigma \odot \xi$. Randomness lives in $\xi$, so gradients flow through $\mu$ and $\sigma$ back to $\phi$ and $\psi$.
</Figure>
...
Of course, since we still have an expectation, we will have to take multiple samples of $z^{(i)}$ in case of $\theta$, and $\xi^{(i)}$ in case of $\psi$ and $\phi$ and then take an average over the derivatives.
...
For convenience, I will use $\hat{z}^{(i)}$ as the reparameterized $z^{(i)}$. But of course, $\hat{z}^{(i)} = z^{(i)}$. Now plugging $\hat{z}^{(i)}$ into the equation, we get:
...
The solution for the derivative of $\text{ELBO}\left(x^{(i)}; \, \phi, \psi, \theta \right)$ with respect to $\psi$ is similar.
</Derivation>